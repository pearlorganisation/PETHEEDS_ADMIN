...
import { getAllBookings, updateOrderCompletion } from '../../features/actions/booking';
...
const [viewData,setViewData]= useState()
  ...
  const handleSubmit = (event, orderId) => {
    ...
    dispatch(updateOrderCompletion({ orderId, isCompleted: approval[orderId] }));
  };
  ...
  useEffect(() => {
    if (Array.isArray(bookingData?.data) && bookingData?.data?.length > 0) {
      ...
      bookingData?.data?.forEach((item) => {
        initialApproval[item._id] = item.orderStatus !== null ? item.orderStatus : null;
      });
      ...
      <div className="max-w-screen-xl mx-auto px-4 md:px-8">
        <div className="justify-between md:flex">
            ...
            <select
                          ...
                          <button 
                          onClick={()=>setFilter("")}
                           className='text-xs bg-red-600 hover:bg-red-500 px-2 py-1 rounded-lg text-white font-semibold'>Remove</button>
            ...
            ></input>
               <div onClick={()=>{getBookings()}} className='bg-blue-600 hover:bg-blue-700 cursor-pointer  text-white rounded-e-lg  py-[10px] px-3 k font-semibold'><IoSearchOutline /></div>
